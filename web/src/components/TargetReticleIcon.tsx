...
export function TargetReticleIcon({ team, size = 'sm', className = '', title }: TargetReticleIconProps) {
  const px = SIZE_PX[size]
  const fill = TEAM_FILL[team]
  return (
    <span className={`reticle-icon reticle-icon-${size} ${className}`} title={title}>
      <svg
        viewBox="0 0 24 24"
        width={px}
        height={px}

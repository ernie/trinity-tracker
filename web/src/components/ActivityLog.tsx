...
export function ActivityLog({
  ...
  const humanPlayersWithServer = useMemo(() => {
    ...
    for (const [, status] of servers.entries()) {
      ...
      const serverName = serverDisplay(status.source, status.key, {
        hasMultipleSources,
      });
  ...
  const filteredActivities = useMemo(() => {
    ...
    return activities.filter((a) => {
      const status =
        a.serverId !== undefined ? servers.get(a.serverId) : undefined;
      if (status?.source && hiddenSources.has(status.source)) return false;
      if (a.serverId !== undefined && hiddenServers.has(a.serverId))
        return false;
      if (
        status?.game_type &&
        hiddenGameTypes.has(status.game_type.toLowerCase())
      )
        return false;
        ...
        <div className="active-players-section">
          ...
          <ul className="active-players-list">
            {humanPlayersWithServer.map(({ player, serverName }) => (
              <div
                key={`${serverName}-${player.client_num}`}
                className="active-player-wrapper"
              >
                <PlayerItem
                  player={player}
                  onClick={
        ...
        {filteredActivities.map((activity) =>
          activity.activityType === "match_start" && activity.mapName ? (
            <div
              ...
              style={{
                backgroundImage: `url(/assets/levelshots/${activity.mapName.toLowerCase()}.jpg)`,
              }}
            >
              <div className="activity-map-text">
                <span className="activity-map-name">{activity.mapName}</span>
                <span className="activity-map-time">
                  {activity.timestamp.toLocaleTimeString()}
                </span>
              </div>
              {activity.serverName && (
                <span className="activity-map-server">
                  {activity.serverName}
                </span>
              )}
            </div>
            ...
            <div
              ...
              title={getPlainText(activity.message)}
            >
              ...
              {activity.serverName && (
                <span className="activity-server-watermark">
                  {activity.serverName}
                </span>
              )}
            </div>
...
function ActivityMessage({ activity, onPlayerClick }: ActivityMessageProps) {
    ...
    if (victimIdx !== -1) {
      ...
      victimElement = (
        <span
          ...
          onClick={() =>
            onPlayerClick(victimName, victimCleanName, victimPlayerId)
          }
        >
  ...
  return (
    <>
      ...
      {!icon && !isBot && playerId && (
        <PlayerBadge
          isVerified={activity.player?.isVerified}
          isAdmin={activity.player?.isAdmin}
          isVR={activity.player?.isVR}
        />
      )}
      <span
        className="clickable-player"

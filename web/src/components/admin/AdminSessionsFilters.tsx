...
export function loadAdminSessionsFilters(): AdminSessionsFilterState {
  try {
    ...
    return {
      ...
      playerLabel: typeof parsed.playerLabel === 'string' ? parsed.playerLabel : '',
      datePreset:
...
export function AdminSessionsFilters({ servers, state, onChange, token }: Props) {
    ...
    <div className="admin-filters-panel">
      ...
      <div className="static-section static-section--row">
        <PlayerSection
          ...
          onPick={(p) =>
            onChange({ ...state, playerId: p?.id ?? null, playerLabel: p?.clean_name ?? '' })
          }
        />
        <DateRangeSection
  ...
  return (
    ...
    >
      {bySource.map(([source, sourceServers]) => {
        const hiddenCount = sourceServers.filter((s) => hidden.includes(s.id)).length
        const sourceVisible = hiddenCount < sourceServers.length
        const indeterminate = hiddenCount > 0 && hiddenCount < sourceServers.length
          ...
          <details key={source} className="filter-subgroup" open>
            <summary className="filter-subgroup__header">
              ...
              <span
                ...
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleSource() }}
              >
                <Switch
...
function PlayerSection({ token, playerId, playerLabel, onPick }: PlayerSectionProps) {
  ...
  return (
    <StaticSection
      ...
      action={
        playerId !== null
          ? { label: 'Clear', onClick: () => { onPick(null); setQuery('') } }
          : undefined
      }
    >
      {playerId !== null ? (
...
export function adminSessionsFiltersActive(s: AdminSessionsFilterState): boolean {
  return (

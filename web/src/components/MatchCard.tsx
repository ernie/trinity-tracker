...
function duelistFromMatchPlayer(
  p: MatchPlayerSummary | undefined,
): DuelistData {
  if (!p) {
    return { name: "—", score: 0 };
  }
...
export function MatchCard({
  ...
  const topCount =
    topScore > 0 ? ffaPlayerData.filter((p) => p.score === topScore).length : 0;
  const ffaGild = isFfa && topCount === 1;
  const spectators = players
    .filter(isSpectator)
    .map((p) => ({ name: p.name, isVR: p.is_vr }));
  ...
  const demoActions = match.demo_url ? (
    <span className="demo-actions">
      ...
      <a
        ...
        className="demo-action download"
      >
      ...
      <div className="card__topbar">
        ...
        {canFeature && (
          <button
            ...
            aria-label={
              isFeatured ? "Unfeature this demo" : "Feature this demo"
            }
      ...
      {isDuel && activeTeams.length >= 2 ? (
        <Duelists
          ...
          state={classifyScores(
            activeTeams[0]?.score ?? 0,
            activeTeams[1]?.score ?? 0,
          )}

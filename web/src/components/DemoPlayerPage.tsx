...
export function DemoPlayerPage() {
  ...
  }, [sfxVolume, musicVolume, voipVolume, muted, engineReady])
  ...
  useEffect(() => { localStorage.setItem('demo_voip_volume', String(voipVolume)) }, [voipVolume])
  ...
  return (
    <div className="demo-player-page">
      ...
      <div className="demo-controls-bar" onContextMenu={e => e.preventDefault()}>
        ...
        <div className="ctrl-group">
          <div ref={volumeWrapRef} className={`ctrl-volume-wrap${volumeOpen ? ' open' : ''}`}>
            ...
            >
              {(() => {
                const peakVol = Math.max(sfxVolume, musicVolume, Math.min(voipVolume, 1))
            ...
            <div className="ctrl-volume-panel">
              ...
              <div className="ctrl-volume-row">
                <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.3-3c0 3-2.54 5.1-5.3 5.1S6.7 14 6.7 11H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c3.28-.48 6-3.3 6-6.72h-1.7z"/></svg>
      ...
      <div className="demo-move-pad" onContextMenu={e => e.preventDefault()}>
        <button className="move-btn" aria-label="Up" {...holdHandlers(() => sendKey('Space', 'keydown'), () => sendKey('Space', 'keyup'))}><ArrowIcon direction="up" size={16} /></button>
        <button className="move-btn" {...holdHandlers(() => sendKey('KeyW', 'keydown'), () => sendKey('KeyW', 'keyup'))}>W</button>
        <button className="move-btn" aria-label="Down" {...holdHandlers(() => sendKey('KeyC', 'keydown'), () => sendKey('KeyC', 'keyup'))}><ArrowIcon direction="down" size={16} /></button>

...
export function HelpModeRoot({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const show = (e: Event) => {
      ...
      if (!target) return
      setPopover({ text: target.getAttribute('data-help')!, rect: target.getBoundingClientRect() })
    }
    ...
    const hide = (e: Event) => {
      const related = (e as MouseEvent | FocusEvent).relatedTarget as (Element | null)
      if (related && typeof related.closest === 'function' && related.closest('[data-help]')) return
      setPopover(null)
    }
...
function HelpPopover({ text, anchorRect }: { text: string; anchorRect: DOMRect }) {
  ...
  useLayoutEffect(() => {
    ...
    if (side === 'above' && rect.top < VIEWPORT_MARGIN) {
      setSide('below')
    } else if (side === 'below' && rect.bottom > window.innerHeight - VIEWPORT_MARGIN) {
  ...
  const style: React.CSSProperties = side === 'above'
    ? { left: centerX, top: anchorRect.top - 8, transform: 'translate(-50%, -100%)' }
    : { left: centerX, top: anchorRect.bottom + 8, transform: 'translateX(-50%)' }

  return createPortal(
    <div ref={ref} className={`help-popover help-popover--${side}`} role="tooltip" style={style}>

...
export function LiveDataProvider({ children }: { children: ReactNode }) {
  ...
  const obeliskAttackersExpiryRef = useRef<Map<number, Map<number, number>>>(
    new Map(),
  );
  const [obeliskAttackersByServer, setObeliskAttackersByServer] = useState<
  ...
  const { activeHumanPlayersCount, activeServersCount } = useMemo(() => {
    ...
    return {
      activeHumanPlayersCount: humans,
      activeServersCount: serversWithHumans,
    };
  }, [servers]);
  ...
  const getServerGameType = useCallback(
    (serverId: number): string | undefined => {
  ...
  const refreshObeliskAttackerState = useCallback(() => {
    ...
    setObeliskAttackersByServer((prev) => {
      ...
      for (const [
        serverId,
        expiryMap,
      ] of obeliskAttackersExpiryRef.current.entries()) {
        ...
        if (!prevSet || prevSet.size !== live.size) {
          changed = true;
          continue;
        }
      ...
      if (!changed)
        for (const serverId of prev.keys())
  ...
  useEffect(() => {
    const id = setInterval(
      ...
      OBELISK_ATTACK_PRUNE_INTERVAL_MS,
    );
    ...
    (event: WSEvent) => {
      switch (event.event) {
          ...
          if (!data.player.is_bot) {
            ...
            setTimeout(() => {
              setNewPlayers((prev) => {
                ...
                n.delete(data.player.clean_name);
                return n;
              });
            }, 60000);
          }
        ...
        case "obelisk_destroy": {
          ...
          addActivity(
            "info",
            `${data.attacker_name}${COLOR_RESET} destroyed the ${teamName} obelisk!`,
            {
              player,
              serverId: event.server_id,
              serverName,
          ...
          if (data.player_id !== undefined) {
            const expiryMap = obeliskAttackersExpiryRef.current.get(
              event.server_id,
            );
            if (expiryMap) {
              const status = serversRef.current.get(event.server_id);
              const slot = status?.players?.find(
                (p) => p.clean_name === cleanName,
              )?.client_num;
              if (slot !== undefined && expiryMap.delete(slot))
                refreshObeliskAttackerState();
            }
          }
          break;
        ...
        case "obelisk_damage": {
          ...
          if (data.active) {
            ...
            addActivity(
              "info",
              `${data.attacker_name}${COLOR_RESET} is attacking the ${teamName} obelisk`,
              {
                player,
                serverId: event.server_id,
                serverName,
                activityType: "obelisk_damage",
                team: data.team,
              },
            );
          }
          ...
          if (data.client_num >= 0) {
            let expiryMap = obeliskAttackersExpiryRef.current.get(
              event.server_id,
            );
            if (data.active) {
              if (!expiryMap) {
                expiryMap = new Map();
                obeliskAttackersExpiryRef.current.set(
                  event.server_id,
                  expiryMap,
                );
              }
              expiryMap.set(
                data.client_num,
                Date.now() + OBELISK_ATTACK_DECAY_MS,
              );
            } else if (expiryMap) {
        ...
        case "skull_pickup": {
          ...
          addActivity(
            "info",
            `${data.player_name}${COLOR_RESET} picked up a ${enemyName} skull (${data.count})`,
            {
              player,
              serverId: event.server_id,
              serverName,
              activityType: "skull_pickup",
              team: data.team,
            },
          );
          break;
        ...
        case "skull_score": {
          ...
          addActivity(
            "info",
            `${data.player_name}${COLOR_RESET} scored ${data.skulls} skull${data.skulls !== 1 ? "s" : ""} for ${teamName}`,
            {
              player,
              serverId: event.server_id,
              serverName,
        ...
        case "team_change": {
          ...
          addActivity(
            "info",
            `${data.player_name}${COLOR_RESET}: ${oldTeam} → ${newTeam}`,
            {
              player,
              serverId: event.server_id,
              serverName,
        ...
        case "say": {
          ...
          addActivity(
            "chat",
            `${data.player_name}${COLOR_RESET}: ${data.message}`,
        ...
        case "say_team": {
          ...
          addActivity(
            "chat",
            `(team) ${data.player_name}${COLOR_RESET}: ${data.message}`,
            { player, serverId: event.server_id, serverName },
          );
          break;
        }
        case "tell": {
          ...
          addActivity(
            "chat",
            `${data.from_name}${COLOR_RESET} -> ${data.to_name}${COLOR_RESET}: ${data.message}`,
            { player, serverId: event.server_id, serverName },
          );
          break;
        }
          ...
          switch (data.award_type) {
            ...
            case "humiliation":
              if (data.victim_name) {
                ...
                const victimBotInfo = getPlayerBotInfo(
                  event.server_id,
                  victimCleanName,
                );
                victim = {
            ...
            case "defend": {
              const gameType = getServerGameType(event.server_id);
              const teamColor =
                data.team === 1 ? "Red" : data.team === 2 ? "Blue" : null;
  ...
  const showPlayer = useCallback(
    (playerName: string, _cleanName: string, playerId?: number) => {

...
func Apply(a *Answers, opts ActuateOptions) error {
	...
	if a.RunsLocalServers() && a.InstallEngine {
		...
		if err := InstallEngine(plan, a.Quake3Dir, "/var/log/quake3"); err != nil {
			return err
		}
...
func installPerServerFiles(plan *Plan, a *Answers, uid, gid int) error {
	...
	if len(a.Servers) > 0 {
		...
		for mod := range mods {
			...
			// autoexec.cfg gets `exec trinity.cfg` only if there's no
			// existing file — never overwrite operator content.

...
// ActuateOptions controls install-time side effects.
type ActuateOptions struct {
...
func Apply(a *Answers, opts ActuateOptions) error {
	if opts.Out == nil {
		opts.Out = os.Stderr
	}
	if opts.ConfigPath == "" {
...
// `trinity init` once the actuator has consumed the staged web
// assets — success or failure. Same /tmp/ guard as stagedWebDir.
func CleanupStage() {
	root := os.Getenv("TRINITY_INIT_STAGE")
	if root == "" {
		return
...
func installLogrotate(plan *Plan) error {
	...
	// The su line uses "quake quake" — the systemd unit owns the log
	// fd and a non-root logrotate run can't truncate a file owned by
	// quake. Hardcoded to "quake" because the systemd unit is too;
	// service-user customization can come back if it's ever needed.
...
// each server's <key>.cfg from its gametype template, the .env file
// with the bind port, and enables the systemd template instance.
//
...
func installPerServerFiles(plan *Plan, a *Answers, uid, gid int) error {
	...
	if len(a.Servers) > 0 {
		...
		for mod := range mods {
			...
			// autoexec.cfg only needs to live in baseq3 — q3's vfs
			// looks up cfg files in the active mod first and falls
			// back to baseq3, so a baseq3/autoexec.cfg that does
			// `exec trinity.cfg` covers missionpack too. Writing one
			// in missionpack would shadow operator customizations
			// they put in baseq3.
			if mod != "baseq3" {
				continue
			}
			autoexec := filepath.Join(a.Quake3Dir, mod, "autoexec.cfg")
	...
	for _, s := range a.Servers {
		// <key>.env: bind port (+ fs_game for missionpack)
		envPath := filepath.Join("/etc/trinity", strings.ToLower(s.Key)+".env")
		...
		// <key>.cfg from gametype template
		cfgPath := filepath.Join(a.Quake3Dir, s.Gametype.ModFolder(), strings.ToLower(s.Key)+".cfg")
		if err := plan.MkdirAll(filepath.Dir(cfgPath), 0755); err != nil {
			return err
		}
		body, err := RenderServerCfg(s.Gametype, s.Key)
		if err != nil {
			return err
		}

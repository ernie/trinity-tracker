...
func ExtractMapLongNames(pk3s []string) (map[string]ArenaMeta, error) {
	...
	for _, pk3Path := range pk3s {
		found, err := ExtractMapLongNamesFromPk3(pk3Path)
		if err != nil {
			return nil, err
		}
		for k, v := range found {
...
// ExtractMapLongNamesFromPk3 reads the worldspawn message of every maps/*.bsp
// in one pk3, keyed by lowercase map id.
func ExtractMapLongNamesFromPk3(pk3Path string) (map[string]ArenaMeta, error) {
	out := make(map[string]ArenaMeta)
	err := IteratePk3(pk3Path, func(name string, open func() (io.ReadCloser, error)) error {

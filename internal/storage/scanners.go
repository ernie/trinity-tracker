...
func scanUser(s scanner) (*User, error) {
	...
	err := s.Scan(&user.ID, &user.Username, &user.PasswordHash, &user.IsAdmin,
		&playerID, &user.PasswordChangeRequired, &user.CreatedAt, &lastLogin, &user.GameToken, &displayName)
	if err != nil {
		return nil, err
	}
...
func scanMatchPlayerSummary(s scanner, includeMatchID bool) (int64, *domain.MatchPlayerSummary, error) {
	...
	if includeMatchID {
		err = s.Scan(&matchID, &ps.PlayerID, &ps.Name, &ps.CleanName, &ps.Frags, &ps.Deaths,
			...
			&ps.Impressives, &ps.Excellents, &ps.Humiliations, &ps.Defends, &ps.Victories, &ps.Captures, &ps.Assists,
			&ps.FlagReturns, &ps.SkullsDelivered, &ps.ObelisksDestroyed, &ps.IsVR,
			&ps.IsVerified, &ps.IsAdmin)
	} else {
